...
function exportExcel() {
  ...
  const rows = state.employees.map((employee) => {
    ...
    return `<tr><td class="rut">${escapeHtml(employee.rut || '')}</td><td class="person">${escapeHtml(employee.name)}</td><td class="${hoursClass}">${formatNumber(employee.hours)}</td><td class="closing">${employee.overnight ? 'SÍ' : 'NO'}</td>${shifts}<td class="signature"></td></tr>`;
  }).join('');
  const roster = `<table class="roster"><colgroup><col class="col-rut"><col class="col-person"><col class="col-hours"><col class="col-closing">${days.map(() => '<col class="col-time">').join('')}<col class="col-signature"></colgroup><thead><tr><th rowspan="2" class="rut">RUT</th><th rowspan="2" class="person">NOMBRE</th><th rowspan="2">HORAS</th><th rowspan="2">TR</th>${dayGroups}<th rowspan="2" class="signature">FIRMA</th></tr><tr>${dayNames}</tr></thead><tbody>${rows}</tbody></table>`;
  const html = `<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Horario</x:Name><x:WorksheetOptions><x:Selected/><x:PageSetup><x:Layout x:Orientation="Landscape"/><x:PageMargins x:Bottom="0.25" x:Left="0.25" x:Right="0.25" x:Top="0.25"/></x:PageSetup><x:FitToPage/><x:FitToWidth>1</x:FitToWidth><x:FitToHeight>1</x:FitToHeight></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><style>@page{size:landscape;margin:.25in}body{font-family:Arial,Aptos,"Segoe UI",sans-serif;color:#111;padding:10px}.document-head{width:100%;margin:0 0 16px;border-collapse:collapse;table-layout:fixed}.document-head td{height:62px;border:0;padding:4px 8px}.store{width:28%;font-size:15px;font-weight:700;text-align:left}.title{width:44%;text-align:center}.title img{display:inline-block;width:48px;height:48px;margin-right:10px;vertical-align:middle}.title strong{display:inline-block;vertical-align:middle;font-size:21px;font-style:italic;text-decoration:underline}.week{width:28%;font-size:11px;font-weight:700;text-align:right}.roster{width:100%;border-collapse:collapse;font-size:9px;table-layout:fixed}.roster .col-rut{width:88px}.roster .col-person{width:205px}.roster .col-hours{width:26px}.roster .col-closing{width:32px}.roster .col-time{width:80px}.roster .col-signature{width:165px}.roster th,.roster td{height:27px;border:1px solid #111;padding:3px;text-align:center;mso-number-format:"\\@"}.roster th{background:#632b2e;color:#fff;font-weight:800}.roster .date{background:#7b3638;font-size:10px}.roster .day{font-size:9px}.roster .rut{font-size:9px;white-space:nowrap}.roster .person{text-align:left;font-size:10px;font-weight:700}.roster .hours{background:#d9f1d2;color:#176b2c;font-size:10px}.roster .hours-low{background:#ffd6d6;color:#b51224}.roster .closing{background:#ffd6d6;color:#a90f1f}.roster .shift{background:#fff;font-size:10px;white-space:nowrap}.roster .free{background:#000}.roster .signature{background:#fff;color:#111}</style></head><body><table class="document-head"><tr><td class="store">Tienda: ${storeName}</td><td class="title"><img src="${burgerKingLogoPng}" alt="Burger King"><strong>Horario crew</strong></td><td class="week">Semana del ${escapeHtml(weekLabel())}</td></tr></table>${roster}</body></html>`;
