...
function exportExcel() {
  ...
  const rows = state.employees.map((employee, index) => {
    ...
    return `<tr><td class="number">${index + 1}</td><td class="person">${escapeHtml(employee.name)}<small>${escapeHtml(employee.rut || 'Sin RUT')}</small></td><td class="${hoursClass}">${formatNumber(employee.hours)}</td><td class="closing">${employee.overnight ? 'SÍ' : 'NO'}</td>${shifts}<td colspan="2" class="signature"></td></tr>`;
  }).join('');
  const roster = `<table class="roster"><colgroup><col class="col-number"><col class="col-person"><col class="col-hours"><col class="col-closing">${days.map(() => '<col class="col-time">').join('')}<col class="col-signature"><col class="col-signature"></colgroup><thead><tr class="sheet-meta"><td colspan="4" class="store">Tienda: ${storeName}</td><td colspan="3" class="title-cell"><img src="${burgerKingLogoPng}" alt="Burger King"><strong>Horario crew</strong></td><td colspan="6" class="week">Semana del ${escapeHtml(weekLabel())}</td></tr><tr><th rowspan="2" class="number">N°</th><th rowspan="2" class="person">NOMBRE Y RUT</th><th rowspan="2">HORAS</th><th rowspan="2">TR</th>${dayGroups}<th rowspan="2" colspan="2" class="signature">FIRMA</th></tr><tr>${dayNames}</tr></thead><tbody>${rows}</tbody></table>`;
  const html = `<!doctype html><html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Horario</x:Name><x:WorksheetOptions><x:Selected/><x:PageSetup><x:Layout x:Orientation="Landscape"/><x:PageMargins x:Bottom="0.25" x:Left="0.25" x:Right="0.25" x:Top="0.25"/></x:PageSetup><x:FitToPage/><x:FitToWidth>1</x:FitToWidth><x:FitToHeight>1</x:FitToHeight></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><style>@page{size:landscape;margin:.25in}body{font-family:Arial,Aptos,"Segoe UI",sans-serif;color:#111;padding:10px}.roster{width:100%;border-collapse:collapse;font-size:9px;table-layout:fixed}.roster .col-number{width:26px}.roster .col-person{width:205px}.roster .col-hours{width:26px}.roster .col-closing{width:32px}.roster .col-time{width:80px}.roster .col-signature{width:82px}.roster th,.roster td{height:27px;border:1px solid #111;padding:3px;text-align:center;mso-number-format:"\\@"}.roster .sheet-meta td{height:76px;border:0;background:#fff;color:#111}.roster .sheet-meta .store{padding-left:8px;font-size:15px;font-weight:700;text-align:left}.roster .sheet-meta .title-cell{text-align:center}.roster .sheet-meta .title-cell img{display:block;width:42px;height:42px;margin:0 auto 3px}.roster .sheet-meta .title-cell strong{display:block;font-size:19px;font-style:italic;text-decoration:underline}.roster .sheet-meta .week{padding-right:8px;font-size:11px;font-weight:700;text-align:right}.roster th{background:#632b2e;color:#fff;font-weight:800}.roster .date{background:#7b3638;font-size:10px}.roster .day{font-size:9px}.roster .number{font-size:9px}.roster .person{text-align:left;font-size:10px;font-weight:700}.roster .person small{display:block;margin-top:2px;font-size:8px;font-weight:400}.roster .hours{background:#d9f1d2;color:#176b2c;font-size:10px}.roster .hours-low{background:#ffd6d6;color:#b51224}.roster .closing{background:#ffd6d6;color:#a90f1f}.roster .shift{background:#fff;font-size:10px;white-space:nowrap}.roster .free{background:#000}.roster .signature{background:#fff;color:#111}</style></head><body>${roster}</body></html>`;
